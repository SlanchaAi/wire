...
- `wire up [<relay>]` — **the one command you run.** One-shot bootstrap: mint identity + bind federation relay + claim + opportunistic local dual-bind + background daemon. The fastest fresh-box-to-ready path. Takes a relay URL or bare host (`wire up @wireup.net` / `wire up http://127.0.0.1:8771`); your handle is DID-derived per the one-name rule, never typed. `--offline` mints the keypair only (no relay bound — bind later); `--with-local <url>` overrides the default `127.0.0.1:8771` local probe; `--no-local` skips it.
- `wire init` — *internal primitive*: mints the Ed25519 keypair (your DID-derived persona is derived from it — init is the sole naming event) and binds a mailbox slot. You never run this directly; `wire up` wraps it (and `wire up --offline` covers offline keygen). No handle to type.
