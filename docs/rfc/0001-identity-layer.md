...
- **O4 — Auto-pair toggle UX.** Per-org policy stored where (`config/wire/org_policies.json`) and surfaced how (CLI: `wire org policy <org_did> --auto-pair`; MCP: new `wire_org_set_policy` tool requiring explicit user consent like `wire_accept`)? **Owner:** swift-harbor.
