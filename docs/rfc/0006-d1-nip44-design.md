...
- **Generate:** at `pair_session.rs::init_self_idempotent` (keypair gen `:593`, card build `:606`). Mint the X25519 keypair *together with* identity so card `dh_pubkey` and stored `x25519.key` are co-minted. Derive per §1a from the Ed25519 identity seed so it is stable + re-derivable; store the private scalar at `config_dir()/x25519.key` mode 0600 (§5).
- **Emit on card:** `agent_card.rs::build_agent_card` (`:205-221`) — add top-level `"dh_pubkey": b64(x25519_pub)`, **parallel to** `verify_keys`, never inside it (`PROTOCOL.md:71`). `card_canonical` (`:432-434`) hashes all fields except `signature`, so adding it before `sign_agent_card` (`:438`) makes the self-signature cover it automatically — **no signing change**. Add reader `card_dh_pubkey(card) -> Option<&str>` next to `card_op_did` (`:400`).
- **Exchange at pairing — zero new payload field:** `pair_session.rs::pair_session_finalize` seals `bootstrap_payload {card, relay_url, slot_id, slot_token}` at `:411-416`. Since `card` already carries `dh_pubkey`, the X25519 key rides the existing **SPAKE2/SAS-authenticated, AEAD-sealed** bootstrap inside the signed card. **Do NOT** key D1 from `pair_session.rs::aead_key` (`:316`) — that secret stays bootstrap-only (RFC-006 open-Q resolution, `0006-...:102`).
- **Pin per peer:** receiver verifies the card (`:444`, `verify_agent_card`) and pins via `add_agent_card_pin` (`trust.rs:156-200`), which stores the **entire** peer card under `agents[handle].card` (`:197`) ⇒ `dh_pubkey` is automatically persisted in `trust.json` and SAS-authenticated by construction. Read at derivation time via `trust["agents"][handle]["card"]["dh_pubkey"]`.
...
3. **X25519 key mint + storage:** `x25519.key` (0600) co-minted at `init_self_idempotent` (`pair_session.rs:593/606`), derived per §1a from the Ed25519 seed.
...
2. **X25519 key derivation source.** §1a derives the X25519 scalar from the Ed25519 seed (one root secret, clean + re-derivable) but **cryptographically couples** the DH key to the signing key — a compromise of one is a compromise of both, and scalar reuse across primitives is non-standard (cf. SLIP-0010's per-curve-separation rationale, [`0007-spike-curve-derivation.md`](./0007-spike-curve-derivation.md)). Alternative: an independent random X25519 key stored separately (stronger separation, one more secret to manage/back-up/rotate). Lean derive-from-seed for v0.2 simplicity. **Needs a persona/security pass before locking.** Owner: @laulpogan + security review.
...
- In-tree code: `src/signing.rs:39,133,199-230,261-313,469-507`, `src/pull.rs:21-32,147-164,230,262,285-307`, `src/agent_card.rs:189-222,400,432-449`, `src/trust.rs:156-200`, `src/pair_session.rs:316,411-416,440-448,593-606`, `src/cli.rs:4394-4406,5169-5175,9086`, `src/mcp.rs:1425-1438`, `src/config.rs:31,53-60`, `docs/PROTOCOL.md:71,141-157`. [internal, primary]
