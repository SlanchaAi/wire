...
- **`Tier::OrgVerified` ceiling holds.** SSO attestation *never* mints `VERIFIED`. The `wire add` / `wire accept` gesture (or an invite-URL accept) remains the ONLY VERIFIED path. Property-tested in `tests/trust_ceiling_prop.rs` — extend it.
