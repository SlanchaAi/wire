...
| E3-bleed | re-dial inherited the entry's top-level token (a stale LOCAL token after add-peer-slot) for the federation endpoint + clobbered the local endpoint → fed delivery 401 | 🟡 rc8 | cmd_add re-pin now additive; fed token seeded only from a prior FEDERATION endpoint, never top-level/local (glossy pinpointed the dial re-pin path) |
