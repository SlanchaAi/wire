...
- **`wire upgrade` ≡ `wire update`** (commits a511a77 + merge): one command (operator: "merge them"). ALWAYS checks crates.io; installs a newer release if one exists (`cargo install` when a toolchain is present, else download + SHA-256-verify + self-replace the prebuilt binary — toolchain-free, for the Windows agents), then does the atomic daemon swap. No newer version → skips install, still restarts. `--check` reports available-update + drift without acting; `--local` skips the crates.io check (offline / dev build). `update` is a clap visible-alias of `upgrade`. Verified live against crates.io. **Windows agents' path** — they have no cargo, so the prebuilt branch matters; needs a real on-Win10 run before shipping.
