...
- **Group chat I1** (commits fed60da model + 206896e commands/e2e): `wire group create/add/send/tail/list` over a creator-signed roster (`src/group.rs`). Creator-broadcast among VERIFIED members; members receive + tail by group_id. 8 unit + 1 e2e green. **Scope boundary found:** bidirectional chat (members posting to the group) needs introduce-on-vouch (members pin each other at `Introduced` on the creator_sig vouch + the group-msg accept path honoring it) — a security-sensitive trust-gate change, deferred to I2. See `docs/superpowers/specs/2026-05-25-group-chat-design.md` → Topology.
