...
- **`wire upgrade` accumulating daemons on Windows fixed (critical).** Repeated `wire upgrade` spawned a fresh daemon without killing the old ones (glossy-magnolia saw 2→3→4→5 over three cycles — real multiple daemons racing the pull cursor). Root cause: `find_processes_by_cmdline("wire daemon")`'s Windows CIM query matched the literal `wire daemon`, which never matches the actual `wire.exe daemon` command line (the `.exe ` breaks the contiguous match), so upgrade found nothing to kill. Now matches the wire image by `Name` and the role (`daemon` / `relay-server`, the pattern minus a leading `wire `).
