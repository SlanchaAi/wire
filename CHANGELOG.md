...
- **`wire tail` no longer prints raw ciphertext for encrypted DMs it can't decrypt** (#281): an `enc=wire-x25519.v1` body that this build couldn't open (decrypt failed, no key, or an unknown `enc` scheme) was rendered as its raw `{"ct":…}` blob with a green `sig ✓` and no warning — an operator on a stale binary saw gibberish and believed they'd read the message. It now renders an explicit `<encrypted DM … run \`wire upgrade\`>` placeholder and sets `decryptable:false` in `--json`. Forward-compatible for any future `enc` discriminator.
