...
fn check_and_heal_self_userinfo_endpoints() -> DoctorCheck {
        ...
        .and_then(Value::as_array_mut)
    {
        endpoints.retain(|ep| {
            ...
            if setup::assert_relay_url_clean_for_publish(url).is_err() {
    ...
    let legacy_url = self_block
        ...
        .unwrap_or("")
        .to_string();
    if !legacy_url.is_empty() && setup::assert_relay_url_clean_for_publish(&legacy_url).is_err() {
